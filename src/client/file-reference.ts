...
/** One path the tree wants referenced in chat (file or directory). */
export interface ChatReferenceTarget {
  /** Absolute path (normalized to `/` separators). */
  path: string
  /** Whether the target is a directory (mention keeps a trailing `/`). */
  isDirectory?: boolean
}

/**
 * The `@` mention text for one reference, relative to the session cwd when the
 * path sits under it, else the absolute path (cross-drive / unrelated roots
 * have no relative form). A directory keeps the trailing `/` that marks it as
 * something to list. Mirrors the "复制相对路径" wording the tree already uses.
 * @param cwd - the session's working directory.
 * @param target - the referenced path.
 * @returns the `@<path>` text to put in the draft (no trailing space).
 */
export function chatReferenceMention(cwd: string, target: ChatReferenceTarget): string {
  const rel = relativePath(cwd, target.path).replaceAll('\\', '/')
  const path = target.isDirectory && !rel.endsWith('/') ? `${rel}/` : rel
  return `@${path}`
}

/**
 * Append ONE file/directory reference to the composer as PLAIN TEXT, so multiple
 * references accumulate in the draft (option A — the composer's structured
 * single-chip insert replaces rather than stacks, so the tree's right-click
 * reference appends `@<path>` text instead).
 *
 * Reads the current draft, appends `@<mention>` plus a separating space, and
 * writes it back via `setDraft`. Degrades to a logged no-op when the
 * conversation service, session scope, or input shell is unavailable.
 * @param ctx - the client runtime context.
 * @param scope - the target session (carries the cwd for relative mentions).
 * @param target - the path (+ isDirectory) to reference.
 */
export function appendChatReference(
  ctx: ClientRuntimeContext,
  scope: SidebarTabScope,
  ...
  try {
    ...
    const mention = chatReferenceMention(cwd, target)
    // Append, space-separated, keeping any existing draft text ahead.
    const next = current.trimEnd() === ''
      ? mention.trimEnd()
      : `${current.replace(/\s+$/, '')} ${mention.trimEnd()}`
    input.setDraft(next)
  } catch (error) {
    console.warn('[dsh-codex-project] chat reference append failed:', error)
  }
}
