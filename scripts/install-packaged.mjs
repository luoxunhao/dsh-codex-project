...
for (const profile of profiles) {
  ...
  if (add.status !== 0) {
    ...
    // ("cannot resolve profile bundle"). Drop the entry so the profile starts.
    const dump = run('dsh', ['--profile', profile, '--dump-config'], dir)
    if (dump.status !== 0) {
      console.log('  profile no longer boots — dropping the bundles entry to recover it')
      run('dsh', ['plugin', '--profile', profile, 'remove', pkg.name], dir)
      fail(`add failed and the profile was rolled back to bootable-without-the-plugin:\n${add.out.slice(-2000)}\nFix the cause, then re-run this script.`)
    }
