...
7. **打包产物安装走脚本**：`pnpm install:packaged -- --profile web`（`scripts/install-packaged.mjs`，支持多个 `--profile`、`--dry-run`、`--no-build`）。顺序是 build → pack → detach → add → verify，手工做会踩这两个坑（2026-09-21 都实际撞过，脚本为它们而写）：① `add <tgz>` 盖在既有 link 上只改 package.json，lockfile 仍是 `version: link:…`、node_modules 仍是软链，**装完等于没装**；② `remove` 会清掉依赖与 bundles 行、**却把软链留在盘上**，下一次 `add` 顺着它跨盘建链报 `ERR_PNPM_EPERM`，把 profile 半装。所以必须 remove → 手工 `rm node_modules/@luoxunhao/dsh-codex-project`（删链接本身，别碰目标）→ add。收尾用 `dsh --profile <name> --dump-config` 确认 patch 层真合成了（`patched by @luoxunhao/dsh-codex-project` + 两条自有行），并把装进去的 `lib/` 与刚构建的四个 bundle 逐字节比哈希——**`file:` tarball 在 lockfile 里没有 integrity，同版本号重打不会被认成更新**，只比对 version 会被骗。装完 profile 依赖是 `file:<绝对路径>.tgz`：**tarball 删了 profile 就装不回来**，得留在稳定路径。
