...
- **宿主自带的那批 `@deepseek-ai/dsh-*` 挂在 `dependencies` 而不是 peer**：打包产物安装时 pnpm 会按 `^0.1.6-alpha.2` 从 registry **再装一份**进 profile（profile 是 `nodeLinker: hoisted`；2026-09-21 实测 `dsh-fs / dsh-fs-sandbox / dsh-sandbox / dsh-sandbox-policy / dsh-sandbox-windows-acl / dsh-tools / dsh-user-approval` 七个都在，版本与宿主一致时只是冗余）。**版本一错位，插件的 fs 提供者就是建在另一份拷贝上的**；`link:` 装法没这个现象（直接用宿主自己那份树）。升宿主基线时要么跟着一起升，要么把这批挪成 peer。
